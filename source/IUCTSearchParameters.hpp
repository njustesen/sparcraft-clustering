...
namespace SparCraft
{
    class IUCTSearchParameters;

    namespace IUCTMoveSelect
    {
        enum { HighestValue, MostVisited };
    }
}

class SparCraft::IUCTSearchParameters
{											        // DEFAULT				DESCRIPTION
...
public:
	...
	// default constructor
	IUCTSearchParameters() 
	    : _maxPlayer            (Players::Player_One)
        , _rootMoveSelection    (IUCTMoveSelect::MostVisited)
	    , _timeLimit            (0)
        , _cValue               (1)
        , _maxTraversals        (100)
        , _maxChildren          (10)
    ...
    std::vector<std::vector<std::string> > & getDescription()
    {
        if (_desc.size() == 0)
        {
            ...
            ss << "IUCT";                                                _desc[1].push_back(ss.str()); ss.str(std::string());
