...
void SearchExperiment::addPlayer(const std::string & line)
{
    ...
    { 
		...
		printf("%s\n","Portfolio added as player" + playerID);
	}
    else if (playerModelID == PlayerModels::AlphaBeta)
    {
		...
		printf("%s\n","Alpha Beta added as player" + playerID);
    }
    else if (playerModelID == PlayerModels::UCT)
    {
		...
		printf("%s\n","UCT added as player" + playerID);
    }
    else if (playerModelID == PlayerModels::IUCT)
    {
        ...
        PlayerPtr uctPlayer(new Player_IUCT(playerID, params));
        players[playerID].push_back(uctPlayer); 
		printf("%s\n"," IUCT added as player" + playerID);
    }
	else
    {
	...
	for (size_t p1Player(0); p1Player < players[0].size(); p1Player++)
	{
		...
		for (size_t p2Player(0); p2Player < players[1].size(); p2Player++)
		{
			...
			for (size_t state(0); state < states.size(); ++state)
			{
				...
				std::cout << numWins[p1Player][p2Player];
